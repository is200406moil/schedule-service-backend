...
Стили и JavaScript разнесены по страницам. В шаблонах остаются только разметка и
небольшие JSON-конфигурации, которые сервер безопасно передаёт клиентскому коду.
